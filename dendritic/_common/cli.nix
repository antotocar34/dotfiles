...
let
  ...
  myNvim = actualInputs.my-neovim.packages."${system}".neovim;
...
in {
  imports = [ actualInputs.nix-index-database.homeModules.nix-index ];
  ...
  in {
    ...
    programs = {
      ...
      rbw = {
        enable = true;
        package = actualInputs.acpkgs.packages.${system}.rbw;
        settings = {
          pinentry =
